...
contract TokenVesting is Ownable, ReentrancyGuard {
  ...
  error TokenNotSet();
  error VestingScheduleNotRevoked();
  error NotValidToken();
  error NotSufficientTokens();
  error ZeroAmount();
  error ZeroDuration();
  error SlicePeriodNotCorrect();
  error DurationLessCliff();
  ...
  modifier onlyIfVestingScheduleNotRevoked(bytes32 vestingScheduleId) {
    if (vestingSchedules[vestingScheduleId].revoked) {
      revert VestingScheduleNotRevoked();
    }
    _;
  }
  ...
  function createVestingSchedule(
    ...
    if (_slicePeriodSeconds < 1) {
      revert SlicePeriodNotCorrect();
    }
    if (_duration < _cliff) {
      revert DurationLessCliff();
    }
  ...
  /**
   * @notice Change beneficiary of vesting schedule
   * @param vestingScheduleId the vesting schedule identifier
  ...
  function changeBeneficiary(
    ...
    vestingSchedule.beneficiary = newBeneficiary;
  }
  ...
  function _computeReleasableAmount(
    ...
    // Otherwise, some tokens are releasable.
      ...
      uint256 slicePeriodsInDuration = vestingSchedule.duration /
        vestingSchedule.slicePeriodSeconds;
      uint256 maxPaymentsCount = vestingSchedule.duration == slicePeriodsInDuration * vestingSchedule.slicePeriodSeconds ? slicePeriodsInDuration + 1 : slicePeriodsInDuration + 2;
      uint256 paymentsShouldHaveDone = timeFromStart/vestingSchedule.slicePeriodSeconds +1;
      uint256 vestedAmount = (vestingSchedule.amountTotal * paymentsShouldHaveDone) /
        maxPaymentsCount;
